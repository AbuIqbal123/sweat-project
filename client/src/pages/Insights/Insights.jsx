...
const Insights = () => {
  ...
  const [module, setModule] = useState(params?.id);
  ...
  async function extractInfo(key, name, obj) {
    const { deadline, weightage } = await obj;
    return { key: key, type: name, deadline: "Week" + deadline, weightage };
  }
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const extractedInfoArray1 = await extractInfo(
        counter + 2,
        "Class Test",
        result.data.classtestPrep[0]
      );
      const extractedInfoArray2 = await extractInfo(
        counter + 1,
        "Course Work",
        result.data.courseworkPrep[0]
      );
